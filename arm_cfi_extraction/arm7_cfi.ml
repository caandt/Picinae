  ...
  | di::t ->
    (match dis' with
     ...
     | di'::t' ->
       ...
       (fun x ->
       if if (=) x j then true else (=) x k then ( * ) di' 4 else f' x))
...
let make_jump_table dis dis' ai sl sr n =
  let m = make_jump_table_map dis dis' sl sr (fun _ -> ( * ) ai 4) in
...
let coq_GOTO l cond src dest =
  ...
  if if (<) offset (-8388608) then true else (>) offset 8388607
  then None
...
(** val bx_blx_irm : bool -> int -> coq_IRM **)

let bx_blx_irm l reg cond _ ti sl sr =
  arm_assemble_all_cond
    (List.append (arm_table_lookup ti sl sr reg)
      ((if l then ARM_BLX_r (14, reg) else ARM_BX (14, reg))::[])) cond

(** val rewrite_bx_blx :
    bool -> int -> coq_TableCache -> int list -> (int -> int) -> int -> int
    -> int -> int -> coq_NewInst **)

let rewrite_bx_blx l reg =
  rewrite_w_table (bx_blx_irm l reg)
  ...
  let bc =
    ( * ) 4
      ((fun z -> bitb z 0 + bitb z 1 + bitb z 2 + bitb z 3 + bitb z 4 + bitb z 5 + bitb z 6 + bitb z 7 + bitb z 8 + bitb z 9 + bitb z 10 + bitb z 11 + bitb z 12 + bitb z 13 + bitb z 14 + bitb z 15 + bitb z 16 + bitb z 17 + bitb z 18 + bitb z 19 + bitb z 20 + bitb z 21 + bitb z 22 + bitb z 23 + bitb z 24 + bitb z 25 + bitb z 26 + bitb z 27 + bitb z 28 + bitb z 29 + bitb z 30 + bitb z 31)
        ((fun x y -> ((x mod y) + y) mod y) register_list ((lsl) 1 16)))
  in
...
let rewrite_b_bl l cond imm24 i dis i2i' ai tc =
  ...
  (match coq_GOTOz l cond (i2i' i) dst with
   | Some z -> Some (((z::[]),[]),tc)
   | None ->
     (match coq_GOTOz true cond (i2i' i) ai with
      | Some z -> Some (((z::[]),[]),tc)
      | None -> None))
...
(** val _rewrite :
    ...
    int -> int -> int -> int list -> (int list list * int list list) option **)
...
let rec _rewrite zs tc pol i2i' i ti ai bi txt =
  ...
  then None
        ...
        | [] ->
          (match coq_GOTOz true 14 (i2i' i) ai with
           | Some z -> Some (((z::[])::[]),[])
           | None -> None)
        | z::zs0 ->
...
(** val make_i2i' : int -> int -> int -> int list list -> int -> int **)

let make_i2i' bi bi' ai z's =
  let i's = make_i's z's bi' in
  let ie = (+) bi ( (List.length z's)) in
  let ie' = (+) bi' ( (List.length (List.flatten z's))) in
...
(** val cfi_rw :
    ...
    list * int list list) option **)

let cfi_rw pol code bi bi' ti ai =
  let tc = fun _ -> None in
  (match _rewrite code tc pol (Obj.magic Fun.id) bi ti ai bi code with
   | Some p ->
     let z's,_ = p in
